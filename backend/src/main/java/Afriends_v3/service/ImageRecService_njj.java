...
import Afriends_v3.entity.ImagePostBase_list_wlq;
 ...
 */
@Service
public class ImageRecService_njj {

    private static final Logger logger = LoggerFactory.getLogger(ImageRecService_njj.class);

    @Autowired
    private ImageRecMapper_njj imageRecMapper;
    ...
    public Map<String, Object> generateRecommendations(Long userId, Integer limit,
        ...
        logger.info("开始生成图片推荐 - 用户ID: {}, 推荐数量: {}", userId, limit);
        ...
        try {
            ...
            // 3. 从内存获取所有可推荐的图片内容
            List<ImagePostBase_wlq> allImages = ImagePostBase_list_wlq.getAllImagePostBase();
            ...
            List<RecommendationScore> scores = calculateRecommendationScores(
                    userId, allImages, userSoftTags, softTagWeight, hardTagWeight, popularityWeight);

            // 5. 创建推荐结果
            List<ImageRec_njj> recommendations = createRecommendations(userId, scores, limit);
            ...
            int totalCandidates = allImages.size();
    ...
    private Map<String, Object> generateDefaultRecommendations(Long userId, Integer limit) {
        try {
            // 从内存获取热门图片内容（按点赞数排序）
            List<ImagePostBase_wlq> hotContents = ImagePostBase_list_wlq.getAllImagePostBase();

            if (hotContents == null || hotContents.isEmpty()) {
                Map<String, Object> result = new HashMap<>();
                result.put("success", false);
                result.put("message", "没有可用的图片内容");
                return result;
            }
            ...
            List<ImageRec_njj> recommendations = new ArrayList<>();
            for (int i = 0; i < Math.min(hotContents.size(), limit); i++) {
                ImagePostBase_wlq content = hotContents.get(i);
                ImageRec_njj recommendation = new ImageRec_njj();
                recommendation.setUserId(userId);
                recommendation.setImageContentId(content.getPostId());
    ...
    private List<RecommendationScore> calculateRecommendationScores(Long userId,
            List<ImagePostBase_wlq> allImages,
        ...
        for (ImagePostBase_wlq image : allImages) {
            // 跳过用户自己的内容
            ...
            // 跳过已经推荐过的内容
            if (alreadyRecommendedContentIds.contains(image.getPostId())) {
                logger.debug("跳过已推荐内容: contentId={}, userId={}", image.getPostId(), userId);
                continue;
            }

            // 计算各个组件的得分
            float softScore = calculateSoftTagScore(image, userSoftTagString);
            float hardScore = calculateHardTagScore(image, null);
            float popScore = calculatePopularityScore(image);
            float timeScore = calculateTimeDecayScore(image);
            ...
            scores.add(
                    new RecommendationScore(image.getPostId(), finalScore, softScore, hardScore, popScore, timeScore));
        }
    ...
    private Set<Long> getAlreadyRecommendedContentIds(Long userId) {
        try {
            // 查询用户的所有历史推荐记录
            List<ImageRec_njj> existingRecommendations = imageRecMapper.selectImageRecsByUserId(userId);

            // 提取内容ID并去重
            return existingRecommendations.stream()
                    .map(ImageRec_njj::getImageContentId)
     ...
     * 计算软标签相似度得分
     */
    private float calculateSoftTagScore(ImagePostBase_wlq image, String userSoftTags) {
        if (image.getSoftTags() == null || image.getSoftTags().isEmpty()) {
            return 0.1f; // 基础分数
        }
        ...
        String[] imageSoftTags = image.getSoftTags().split(",");
        String[] userSelectedSoftTags = userSoftTags.split(",");

        // 计算匹配的标签数量
        int matchCount = 0;
        for (String imageTag : imageSoftTags) {
            for (String userTag : userSelectedSoftTags) {
                if (imageTag.trim().equals(userTag.trim())) {
                    matchCount++;
                }
            }
        }
        ...
        float matchRatio = (float) matchCount / Math.max(imageSoftTags.length, userSelectedSoftTags.length);
     ...
     * 计算硬标签匹配度得分
     */
    private float calculateHardTagScore(ImagePostBase_wlq image, String userHardTags) {
        if (image.getHardTags() == null || image.getHardTags().isEmpty()) {
            return 0.1f; // 基础分数
        }
        ...
        String[] imageHardTags = image.getHardTags().split(",");
        String[] userSelectedHardTags = userHardTags.split(",");

        // 计算匹配的标签数量
        int matchCount = 0;
        for (String imageTag : imageHardTags) {
        ...
        float matchRatio = (float) matchCount / Math.max(imageHardTags.length, userSelectedHardTags.length);
     ...
     * 计算热度得分
     */
    private float calculatePopularityScore(ImagePostBase_wlq image) {
        int likeCount = image.getLikeCount() != null ? image.getLikeCount() : 0;
        int collectCount = image.getCollectCount() != null ? image.getCollectCount() : 0;
        int commentCount = image.getCommentCount() != null ? image.getCommentCount() : 0;
     ...
     * 计算时间衰减得分
     */
    private float calculateTimeDecayScore(ImagePostBase_wlq image) {
        if (image.getCreatedAt() == null) {
            return 0.1f; // 基础分数
        }

        long currentTime = System.currentTimeMillis();
        long createTime = image.getCreatedAt().getTime();
     ...
     * 创建推荐记录
     */
    private List<ImageRec_njj> createRecommendations(Long userId, List<RecommendationScore> scores, Integer limit) {
        // 按得分排序
        ...
        for (int i = 0; i < Math.min(scores.size(), limit); i++) {
            ...
            ImageRec_njj recommendation = new ImageRec_njj();
            recommendation.setUserId(userId);
            recommendation.setImageContentId(score.imageContentId);
     ...
     * 保存推荐到数据库
     */
    private void saveRecommendations(List<ImageRec_njj> recommendations) {
        ...
        // 不再删除旧的推荐记录，直接追加新推荐
        // 获取当前最大ID
        Long maxId = imageRecMapper.getMaxId();
        if (maxId == null) {
            maxId = 0L;
        }

        // 保存新推荐
        for (ImageRec_njj recommendation : recommendations) {
            // 生成递增的ID
            if (recommendation.getId() == null) {
                maxId++;
                recommendation.setId(maxId);
            }
            imageRecMapper.insertImageRec(recommendation);
        }
    }
     ...
     * 获取推荐内容的详细信息
     */
    private List<Map<String, Object>> getDetailedRecommendations(List<ImageRec_njj> recommendations) {
        ...
        // 批量获取所有推荐内容的详细信息
        List<Long> imageContentIds = recommendations.stream()
                .map(ImageRec_njj::getImageContentId)
                .filter(Objects::nonNull)
                .collect(Collectors.toList());

        if (imageContentIds.isEmpty()) {
            return detailedRecommendations;
        }

        try {
            // 创建ID到内容的映射
            Map<Long, ImagePostBase_wlq> contentMap = new HashMap<>();
            for (Long contentId : imageContentIds) {
                ImagePostBase_wlq content = ImagePostBase_list_wlq.getImagePostBaseById(contentId);
                if (content != null) {
            ...
            for (ImageRec_njj recommendation : recommendations) {
                ...
                ImagePostBase_wlq imageContent = contentMap.get(recommendation.getImageContentId());

                if (imageContent != null) {
                    detailedRecommendation.put("imageContentId", imageContent.getPostId());
                    detailedRecommendation.put("imageUrl", imageContent.getImageUrls());
                    detailedRecommendation.put("softTags", imageContent.getSoftTags());
                    detailedRecommendation.put("hardTags", imageContent.getHardTags());
                    detailedRecommendation.put("likeCount",
                            imageContent.getLikeCount() != null ? imageContent.getLikeCount() : 0);
                    detailedRecommendation.put("collectCount",
                            imageContent.getCollectCount() != null ? imageContent.getCollectCount() : 0);
                    detailedRecommendation.put("commentCount",
                            imageContent.getCommentCount() != null ? imageContent.getCommentCount() : 0);
                    detailedRecommendation.put("heatScore",
                            imageContent.getHeatScore() != null ? imageContent.getHeatScore() : 0.0);
                    detailedRecommendation.put("score", recommendation.getScore());
                    detailedRecommendation.put("rankOrder", recommendation.getRankOrder());
                    detailedRecommendation.put("createdAt", imageContent.getCreatedAt());
                    detailedRecommendation.put("updatedAt", imageContent.getUpdatedAt());
                    ...
                    // 如果找不到内容，返回基本信息
                    detailedRecommendation.put("imageContentId", recommendation.getImageContentId());
                    detailedRecommendation.put("imageUrl", "图片不存在或已删除");
                    detailedRecommendation.put("score", recommendation.getScore());
                    detailedRecommendation.put("rankOrder", recommendation.getRankOrder());
                }
    ...
    public Map<String, Object> batchGenerateRecommendations(List<Long> userIds, Integer limit,
        ...
        logger.info("开始批量生成图片推荐 - 用户数量: {}, 推荐数量: {}", userIds != null ? userIds.size() : 0, limit);
            ...
            for (Long userId : userIds) {
                try {
                    ...
                    if ((Boolean) userResult.get("success")) {
                        ...
                        @SuppressWarnings("unchecked")
                        List<Map<String, Object>> recommendations = (List<Map<String, Object>>) userResult
                                .get("recommendations");
    ...
    private static class RecommendationScore {
        ...
        RecommendationScore(Long imageContentId, float score, float softTagScore, float hardTagScore,
